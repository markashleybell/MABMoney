...
DECLARE @BackupPath NVARCHAR(MAX) = N'C:\Users\Mark\Dropbox\Cloud Data Backups\MABMoney\03-June-2015 0725\'
DECLARE @Params NVARCHAR(MAX) = N' WITH ( BATCHSIZE = 10000, DATAFILETYPE = ''native'', KEEPIDENTITY, KEEPNULLS )'

--EXEC('BULK INSERT [dbo].[__MigrationHistory] FROM ''' + @BackupPath + '\dbo.__MigrationHistory.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Users] FROM ''' + @BackupPath + '\dbo.Users.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Accounts] FROM ''' + @BackupPath + '\dbo.Accounts.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Budgets] FROM ''' + @BackupPath + '\dbo.Budgets.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Categories] FROM ''' + @BackupPath + '\dbo.Categories.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Categories_Budgets] FROM ''' + @BackupPath + '\dbo.Categories_Budgets.dat''' + @Params)
EXEC('BULK INSERT [dbo].[Transactions] FROM ''' + @BackupPath + '\dbo.Transactions.dat''' + @Params)
GO
